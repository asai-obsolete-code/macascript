	...
	`(defun ,name (,s env ,@args)
	   (let ((,definition ,@body))		;body will be evaluated first
		 (m-compile ,s env ,definition)))))
					;so you can perform some bad behavior on "env". it is intentional
...
(defmaca m-iter-obj (val array body &optional (key (gensym)) own)
  (let ((len (gensym "l"))
	(ref (gensym "ref")))
    `((var ,key)
      (var ,val)
      ...
      (glue for
	    ...
	    (blk 
	     ((= ,val (,ref > ',key))
	      ,(when own `(if (,val > (hasOwnProperty ,key))))
	      ,@body))))))
